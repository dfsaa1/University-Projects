...
void envia_mensagem_servidor() {
    ...
    int var = msgsnd(msg_id,&mensagem,sizeof(mensagem.dados),0);
    exit_on_error(var,"Não é possível enviar mensagem para o servidor");
    sucesso("Mensagem para o servidor enviada");
...
void espera_resposta_servidor() {
    ...
    int var = msgrcv(msg_id,&resposta,sizeof(resposta.dados),getpid(),0);
    exit_on_error(var,"Não é possível ler a resposta do servidor");
    sucesso("Servidor enviou resposta");
...
void pedido() {
    ...
    do {
        ...
        case DESCONHECIDO : erro("C5.1) Não existe registo do utente %d, %s", mensagem.dados.num_utente, mensagem.dados.nome);
                            exit(1);
                            break;
        ...
        case NAOHAENFERMEIRO : erro("C5.1) Não existe enfermeiro na localidade do utente %d, %s",mensagem.dados.num_utente, mensagem.dados.nome);
                                exit(1);
                                break;
        ...
        case VACINADO : erro(" C5.2) O utente %d, %s já foi vacinado",mensagem.dados.num_utente,mensagem.dados.nome);
                        exit(0);
                        break;
        ...
        case EMCURSO :  erro(" C5.2) A vacinação do utente %d, %s já está em curso",mensagem.dados.num_utente,mensagem.dados.nome);
                        exit(0);
                        break;
        ...
        case AGUARDAR :   sucesso(" C5.3) Utente %d, %s, por favor aguarde...",mensagem.dados.num_utente,mensagem.dados.nome);
                          sleep(TEMPO_ESPERA);
                          pedido();
                          break;
      }
...
void cancela_pedido(int sinal) {
    ...
    sucesso("C7.1) O cidadão cancelou a vacinação no processo %d", resposta.dados.cidadao.PID_cidadao);
