...
void init_ipc() {
    ...
    int Mutex_Value = semctl(sem_id,0,SETVAL,1);
    exit_on_error(Mutex_Value, "S1) Semáforo com a Key definida não pode ser iniciado com o valor 1");
...
void init_database() {
    ...
    // • Lê o ficheiro FILE_CIDADAOS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->cidadaos e db->num_cidadaos. Se não o conseguir, dá erro e termina com exit status 1;
    size_t size_cid= read_binary(FILE_CIDADAOS, db->cidadaos,MAX_CIDADAOS*sizeof(Cidadao));
    db->num_cidadaos= size_cid / sizeof(Cidadao);
 
    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
    size_t size_enf= read_binary(FILE_ENFERMEIROS, db->enfermeiros,MAX_ENFERMEIROS*sizeof(Enfermeiro));
    db->num_enfermeiros= size_enf / sizeof(Enfermeiro);
...
void envia_resposta_cidadao() {
    ...
    int envia_resposta=msgsnd(msg_id,&resposta,sizeof(resposta.dados),0);
    exit_on_error(envia_resposta,"Não é possível enviar resposta para o cidadão");
    sucesso("Resposta para o cidadão enviada");
...
void processa_pedido() {
    ...
    int cid,enf;
    int num = -1;
    for (cid = 0; cid<db->num_cidadaos; cid++) {
      if(mensagem.dados.num_utente == db->cidadaos[cid].num_utente &&! strcmp(mensagem.dados.nome, db->cidadaos[cid].nome)) {
         num=cid;
         break;
      }
     }
     ...
     resposta.dados.status = DESCONHECIDO;
         ...
         resposta.dados.cidadao=db->cidadaos[num];
       }
     if (db->cidadaos[num].estado_vacinacao == 2) {
      resposta.dados.status = VACINADO;
     }
     if (db->cidadaos[num].PID_cidadao > 0) {
     resposta.dados.status = EMCURSO;
       } else {
         db->cidadaos[cid].PID_cidadao = mensagem.dados.PID_cidadao;
     }
     sucesso("S5.1) Cidadão %d, %s encontrado, estado_vacinacao=%d, status=%d",mensagem.dados.num_utente, mensagem.dados.nome, db->cidadaos[num].estado_vacinacao, resposta.dados.status);
    ...
    int num_enf=-1;
    for(enf=0;enf<db->num_enfermeiros;enf++){
    if(strcmp(db->cidadaos[num].localidade, db->enfermeiros[enf].CS_enfermeiro+2)==0){
        num_enf=enf;
        break;
        }
        ...
        erro("S5.2) Enfermeiro do CS %s não encontrado na BD Enfermeiros",db->cidadaos[num].localidade);
          }else{
           resposta.dados.status=AGUARDAR;
              sucesso("S5.2) Enfermeiro do CS %s encontrado, disponibilidade=%d, status=%d", db->cidadaos[num].localidade, db->enfermeiros[num_enf].disponibilidade,resposta.dados.status);
        }
    ...
    if(db-> enfermeiros[num_enf].disponibilidade==1){
       if(reserva_vaga(num,num_enf)>0){
    ...
    // sucesso("S6.1) Criado um processo filho com PID_filho=%d", <PID_filho>);
     ...
     if (filho==0) {   // Processo FILHO
       ...
       servidor_dedicado();
        ...
        db->vagas[vaga_ativa].PID_filho=filho;
     }else{
         exit_on_error(filho,"S6.1) Não foi possível criar um novo processo");
     }
    debug(">");
}
...
void servidor_dedicado() {
    debug("<");
    int cidadaocid= db->vagas[vaga_ativa].index_cidadao;
    int enfermeiroenf=db->vagas[vaga_ativa].index_enfermeiro;
    ...
    db->enfermeiros[enfermeiroenf].disponibilidade=0;
    //sem_mutex_up();
    ...
    //sem_mutex_down();
    db->cidadaos[cidadaocid].estado_vacinacao=db->cidadaos[cidadaocid].estado_vacinacao+1;
    //sem_mutex_up();
    // S7.6) Atualiza os dados do cidadão (PID_cidadao = -1) na BD de Cidadãos
    //sem_mutex_down();
    db->cidadaos[cidadaocid].PID_cidadao= -1;
    //sem_mutex_up();
    // S7.6) Atualiza os dados do enfermeiro (incrementa nr_vacinas_dadas) na BD de Enfermeiros;
    //sem_mutex_down();
    db->enfermeiros[enfermeiroenf].nr_vacinas_dadas=db->enfermeiros[enfermeiroenf].nr_vacinas_dadas+1;
    //sem_mutex_up();
    // S7.6) Atualiza os dados do enfermeiro (coloca disponibilidade=1) na BD de Enfermeiros;
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    //sem_mutex_down();
    db->enfermeiros[enfermeiroenf].disponibilidade= 1;
    //sem_mutex_up();
     sucesso("S7.6) Cidadão atualizado na BD para estado_vacinacao=%d, Enfermeiro atualizado na BD para nr_vacinas_dadas=%d e disponibilidade=%d", db->cidadaos[cidadaocid].estado_vacinacao, db->enfermeiros[enfermeiroenf].nr_vacinas_dadas , db->enfermeiros[enfermeiroenf].disponibilidade);
...
void cancela_pedido() {
    ...
    for(int c=0;c<db->num_cidadaos;c++){
        if(mensagem.dados.num_utente== db->cidadaos[c].num_utente &&! strcmp(mensagem.dados.nome, db->cidadaos[c].nome)){
            numero_cid=c;
    ...
    for(int k=0;k<MAX_VAGAS;k++){
        if(db->vagas[k].index_cidadao==numero_cid){
            numero_enf=k;
            break;
        }
    }
    ...
    if(numero_enf<0){
        ...
        pid_filho=db->vagas[numero_enf].PID_filho;
        sucesso(" S10.1) Foi encontrada a sessão do cidadão %s, %s na sala com o index %d",mensagem.dados.num_utente,mensagem.dados.nome,numero_enf);
    }
...
void termina_servidor(int sinal) {
    ...
    kill(db->vagas[k].PID_filho,SIGTERM);
        }
    }
